...
let add_tactic_notation local n ?deprecation prods e =
  ...
  let kername = make_fresh_key prods in
  let full_name = KerName.to_string kername in
  Feedback.msg_notice (str "Vernac Begin TacticNotation");
  let pr_prod = function
    | TacTerm s -> str s 
    | TacNonTerm(_, (_, id)) ->
      match id with
      | Some id -> str "(" ++ Id.print id ++ str ")"
      | None -> str "_"
  in
  Feedback.msg_notice (str "Name: " ++ str full_name);
  Feedback.msg_notice (str "LocalName: " ++ prlist_with_sep spc pr_prod prods);
  Feedback.msg_notice (str "Body: ");
  Feedback.msg_notice (Pptactic.pr_glob_tactic (Global.env()) tac);
  Feedback.msg_notice (str "Vernac End TacticNotation");
...
let register_ltac local ?deprecation tacl =
  ...
  | NewTac id ->
    ...
    Feedback.msg_notice (str "Vernac Begin Ltac");
    let full_path = Lib.make_path id in
    let full_name = Libnames.string_of_path full_path in
    Feedback.msg_notice (str "Name: " ++ str full_name);
    Feedback.msg_notice (str "Body: ");
    Feedback.msg_notice (Pptactic.pr_glob_tactic env tac);
    Feedback.msg_notice (str "Vernac End Ltac");
    Flags.if_verbose Feedback.msg_info (Id.print id ++ str " is defined")
  | UpdateTac kn ->
    Tacenv.redefine_ltac local kn tac ?deprecation;
    Feedback.msg_notice (str "Vernac Begin Ltac");
    Feedback.msg_notice (str "Name: " ++ pr_qualid (Tacenv.shortest_qualid_of_tactic kn));
    Feedback.msg_notice (str "Body: ");
    Feedback.msg_notice (Pptactic.pr_glob_tactic env tac);
    Feedback.msg_notice (str "Vernac End Ltac");
