    ...
    | GlobRef.ConstRef const ->
        ...
        (match cb.const_body with
         | Declarations.Def body ->
             ...
             Pretyping.print_constr_title "Arguments";
             let full_path = Nametab.path_of_global sr in
             let full_name = Libnames.string_of_path full_path in
             Printf.printf "Name: %s\n\n" full_name;
             Printf.printf "Type: \n";
             Pretyping.print_type_func env sigma (Names.Id.of_string "_arguments") body_econstr;        
             Pretyping.print_constr_end "Arguments"
         | _ -> ())
    | _ -> ()
  in
  print_args_info ();
