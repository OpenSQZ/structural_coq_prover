...
(* let tok_to_string : Tok.t -> string = function
| Tok.KEYWORD s -> "KEYWORD '" ^ s ^ "'"
| Tok.PATTERNIDENT s -> "PATTERNIDENT '" ^ s ^ "'"
| Tok.IDENT s -> "IDENT '" ^ s ^ "'"
| Tok.FIELD s -> "FIELD '" ^ s ^ "'"
| Tok.NUMERAL n -> "NUMBER '" ^ NumTok.to_string n ^ "'"
| Tok.STRING s -> "STRING \"" ^ s ^ "\""
| Tok.LEFTQMARK -> "LEFTQMARK"
| Tok.BULLET s -> "BULLET '" ^ s ^ "'"
| Tok.QUOTATION (lbl, s) -> "QUOTATION \"" ^ lbl ^ ": " ^ s ^ "\""
| Tok.EOI -> "EOI" *)
...
let important_keywords = [
  (* "Lemma"; "Definition" *)
  "Lemma"; "Theorem"; "Definition"; "Fixpoint"; "Inductive"; "CoInductive"; "Hypothesis";
  "Instance"; "Class"; "Record"; "Structure"; "Axiom"; "Corollary"; "Parameter"; "Variable"; "Notation"
]

let last_token = ref None

let loc_to_global_string loc =
  let start_pos = loc.Loc.bp in
  let end_pos = loc.Loc.ep in
  Printf.sprintf "characters %d-%d" start_pos end_pos

let print_token loc token = ()
  (* match token with
  (* | KEYWORD k ->
      Printf.printf "Keyword: %s \n" k;
      last_token := Some (KEYWORD k) *)
  | IDENT id when List.mem id important_keywords ->
      last_token := Some (KEYWORD id);  (* Treat important IDENTs as KEYWORDs *)
  | IDENT id ->
      (match !last_token with
       | Some (KEYWORD k) when List.mem k important_keywords ->
           Printf.printf "Found %s: %s at %s\n" k id (loc_to_global_string loc);
           flush stdout;
           last_token := None  (* Reset last_token to avoid double printing *)
       | _ -> last_token := Some token)
  | _ -> last_token := Some token *)
...
let rec next_token ~diff_mode loc s =
  ...
  let (token, newloc) as result = 
    ...
    | Some '?' ->
        ...
        comment_stop bp;
        print_token (set_loc_pos loc bp ep) t;
    ...
    | Some ('a'..'z' | 'A'..'Z' | '_' as c) ->
        ...
        begin 
          try 
            let t = find_keyword loc id bp s in
            print_token (snd t) (fst t); (* Print found keyword *)
            t
          with Not_found ->
            let ep = Stream.count s in
            let t = IDENT id, set_loc_pos loc bp ep in
            print_token (snd t) (fst t); (* Print found identifier *)
            t
        end
    | Some ('0'..'9') ->
        ...
        let ep = Stream.count s in
        comment_stop bp;
        let t = (NUMERAL n, set_loc_pos loc bp ep) in
        print_token (snd t) (fst t);
        t
    | Some '\"' ->
        Stream.junk s;
        ...
        let ep = Stream.count s in
        comment_stop bp;
        let t = (STRING (get_buff len), set_loc_pos loc bp ep) in
        print_token (snd t) (fst t);
        t
    | Some ('(' as c) ->
        Stream.junk s;
        begin try
          ...
          | Some '*' when diff_mode ->
              Stream.junk s;
              let ep = Stream.count s in
              let t = (IDENT "(*", set_loc_pos loc bp ep) in
              print_token (snd t) (fst t);
              t
          | Some '*' ->
              Stream.junk s;
          ...
          | _ -> 
            let t = process_chars ~diff_mode loc bp c s in comment_stop bp; 
            print_token (snd t) (fst t); 
            t
        with Stream.Failure -> raise (Stream.Error "")
        end
    ...
    | _ ->
        match lookup_utf8 loc s with
          ...
          | EmptyStream ->
              comment_stop bp; 
              let t = (EOI, set_loc_pos loc bp (bp+1)) in
              print_token (snd t) (fst t);
              t
  in
  (* Printf.printf "Token: %s at %s\n" (tok_to_string token) (loc_to_global_string newloc);
  flush stdout; *)
  result
  (* (match token with
   | KEYWORD "Lemma" ->
       Printf.printf "Found Lemma at %s\n" (loc_to_global_string newloc);
       flush stdout
   | _ -> ());
  result *)
