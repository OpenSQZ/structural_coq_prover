...
let do_assumptions ~program_mode ~poly ~scope ~kind nl l =
  ...
  let (sigma,_,_),l = List.fold_left_map (fun (sigma,env,ienv) (is_coe,(idl,c)) ->
    ...
    List.iter (fun {CAst.loc; v=id} ->
      Pretyping.print_constr_title "Assumption";
      let full_path = Lib.make_path id in
      let full_name = Libnames.string_of_path full_path in
      Printf.printf "Name: %s\n\n" full_name;
      Printf.printf "Type: \n";
      Pretyping.print_type_func env sigma (Id.of_string("_assumption")) t;
      Pretyping.print_constr_end "Assumption"
    ) idl;
...
let context ~poly l =
  ...
  List.iter (fun decl ->
    ...
    let value = Context.Rel.Declaration.get_value decl in
    Pretyping.print_constr_title "Context";
    (match name with
    | Anonymous -> 
        Printf.printf "Name: _Anonymous\n"
    | Name id -> 
        let full_path = Lib.make_path id in
        let full_name = Libnames.string_of_path full_path in
        Printf.printf "Name: %s\n" full_name);
    ...
    Printf.printf "Type: \n";
    Pretyping.print_type_func env sigma (Id.of_string("_context")) typ;
    
    (match value with
    | Some body -> 
        Printf.printf "Body: \n";
        Pretyping.print_type_func env sigma (Id.of_string("_context_body")) body
    | None -> 
        Printf.printf "Body: <no body>\n");
    
    Pretyping.print_constr_end "Context"
  ) ctx;
