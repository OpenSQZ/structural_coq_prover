...
let do_mutual_inductive ~template udecl indl ~cumulative ~poly ~private_ind ~uniform finite =
  ...
  List.iter (fun ind_entry ->
    (* Print inductive type *)
    Pretyping.print_constr_title "Inductive";
    let full_path = Lib.make_path ind_entry.Entries.mind_entry_typename in
    let full_name = Libnames.string_of_path full_path in
      ...
      List.fold_right
        (fun param acc ->
          ...
          | Context.Rel.Declaration.LocalAssum(name, typ) ->
            Constr.mkProd(name, typ, acc)
          | Context.Rel.Declaration.LocalDef(name, body, typ) ->
            Constr.mkLetIn(name, body, typ, acc))
        mie.mind_entry_params
        ind_entry.Entries.mind_entry_arity
    in
    Printf.printf "Type: \n";
    Pretyping.print_type_func env sigma (Id.of_string("_inductive"))
      (EConstr.of_constr full_type);
    Printf.printf "\n";
    
    (* Print constructors *)
    Printf.printf "@IND ";
    Pretyping.print_constr_title "Constructors";
    List.iter2 (fun consname constype ->
      Printf.printf "@@IND Constructor Begin\n\n";
      let cons_full_name = Printf.sprintf "%s.%s" full_name (Names.Id.to_string consname) in
      Printf.printf "Name: %s\n\n" cons_full_name;
      Printf.printf "Type: \n";
      Pretyping.print_type_func env sigma (Id.of_string("_sub"))
        (EConstr.of_constr constype);
      Printf.printf "@@IND Constructor End\n"
    ) ind_entry.Entries.mind_entry_consnames ind_entry.Entries.mind_entry_lc;
    Printf.printf "@IND ";
    Pretyping.print_constr_end "Constructors";
  ) mie.mind_entry_inds;
  Pretyping.print_constr_end "Inductive";
