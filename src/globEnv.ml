...
let print_rel_context_stack env =
  let ctx = Environ.rel_context env in
  let ctx_str = List.mapi (fun i decl ->
    match Context.Rel.Declaration.get_name decl with
    | Name id -> Printf.sprintf "%d: %s" i (Id.to_string id)
    | Anonymous -> Printf.sprintf "%d: <anonymous>" i
  ) ctx in
  Printf.printf "Current rel_context stack:\n%s\n\n" (String.concat "\n" (List.rev ctx_str))


let push_rel ~hypnaming sigma d env =
  (* Printf.printf "\n"; *)
  (* Printf.printf "Before push_rel:\n"; *)
  (* print_rel_context_stack env.static_env; *)
...
let push_rel_context ~hypnaming ?(force_names=false) sigma ctx env =
  ...
  (* Printf.printf "Before push_rel_context:\n";
  print_rel_context_stack env.static_env; *)
  ...
  (* Printf.printf "After push_rel_context:\n";
  print_rel_context_stack env.static_env; *)
  ctx', env
