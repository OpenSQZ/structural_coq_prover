...
let do_primitive id prim typopt =
  ...
  Pretyping.print_constr_title "Primitive";
  let full_path = Lib.make_path id.CAst.v in
  let full_name = Libnames.string_of_path full_path in
  Printf.printf "Name: %s\n" full_name;
  (match typopt with
   | Some typ -> 
      Printf.printf "Type: \n";
      Pretyping.print_type_func env evd (Names.Id.of_string("_primitive")) (EConstr.of_constr typ)
   | None -> 
      Printf.printf "Type: <no body>\n");
  Pretyping.print_constr_end "Primitive";
