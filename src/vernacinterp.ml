...
let rec interp_expr ~atts ~st c =
  ...
  (* Printf.printf "\n========= Begin Vernac expression: =========\n";
  (* Printf.printf "%s\n" (Pp.string_of_ppcmds (Ppvernac.pr_vernac_expr_subset c)); *)
  Printf.printf "%s\n" (Pp.string_of_ppcmds (Ppvernac.pr_vernac_expr c));
  Printf.printf "\n========= End Vernac expression: =========\n"; *)
  let _ = Ppvernac.pr_vernac_expr_subset c in
