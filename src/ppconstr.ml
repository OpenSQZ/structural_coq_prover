...
let tag_var = tag Tag.variable
  ...
  let pr_binder_context many pr (nal,k,t) =
    match k with
      | Generalized (b', t') ->
        begin match nal with
          |[{loc; v=Anonymous}] ->
          ...
          |[{loc; v=Name id}] ->
            ...
            hov 1 (str "`" ++ (surround_impl b'
                                  (str (Libnames.string_of_path full_path) ++ str " : " ++
                                    (if t' then str "!" else mt()) ++ pr t)))
          |_ -> anomaly (Pp.str "List of generalized binders have always one element.")
        end
      | Default b ->
        match t with
          | { CAst.v = CHole (_,IntroAnonymous,_) } ->
            let pr_name name = match name.CAst.v with
            | Name id -> 
                let full_path = Lib.make_path id in
                str (Libnames.string_of_path full_path)
            | Anonymous -> str "_Anonymous"
            in
            let s = prlist_with_sep spc pr_name nal in
            hov 1 (surround_implicit b s)
          | _ ->
            let pr_name name = match name.CAst.v with
            | Name id -> 
            ...
            | Anonymous -> str "_Anonymous"
            in
            let s = prlist_with_sep spc pr_name nal ++ str " : " ++ pr t in
            hov 1 (if many then surround_impl b s else surround_implicit b s)
      
  let pr_binder_among_many_context pr_c = function
    | CLocalAssum (nal,k,t) ->
      pr_binder_context true pr_c (nal,k,t)
    | CLocalDef (na,c,topt) ->
      let pr_name name = match name.CAst.v with
      | Name id -> 
          let full_path = Lib.make_path id in
  ...
  let pr_undelimited_binders_context sep pr_c =
    prlist_with_sep sep (pr_binder_among_many_context pr_c)
  ...
  let pr_binders_context env sigma = pr_undelimited_binders_context spc (pr_expr env sigma ltop)
