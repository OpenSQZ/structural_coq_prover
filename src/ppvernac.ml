...
let string_of_theorem_kind = let open Decls in function
  ...
  let pr_statement head (idpl,(bl,c)) =
    let env = Global.env () in
    let sigma = Evd.from_env env in
    let (lid, univdecl) = idpl in
    let id = lid.CAst.v in
    let full_path = Lib.make_path id in
    hov 2
      (head ++ spc() ++
        str (Libnames.string_of_path full_path) ++
        pr_universe_decl univdecl ++ spc() ++
...
let print_env env =
  ...
  let sigma = Evd.from_env env in
  Printf.printf "\n=== Environment ===\n";
  Printf.printf "Named context:\n";
  List.iter (fun decl ->
    let id = Context.Named.Declaration.get_id decl in
    let ty = EConstr.of_constr (Context.Named.Declaration.get_type decl) in
    match Context.Named.Declaration.get_value decl with
    | None ->
        Printf.printf "  %s : %s (assumption)\n"
          (Names.Id.to_string id)
          (Pp.string_of_ppcmds (Printer.pr_econstr_env env sigma ty))
    | Some body ->
        Printf.printf "  %s := %s : %s (definition)\n"
          (Names.Id.to_string id)
          (Pp.string_of_ppcmds (Printer.pr_econstr_env env sigma (EConstr.of_constr body)))
          (Pp.string_of_ppcmds (Printer.pr_econstr_env env sigma ty))
  ) named_context;
  Printf.printf "Rel context:\n";
  List.iter (fun decl ->
    let name = Context.Rel.Declaration.get_name decl in
    let ty = EConstr.of_constr (Context.Rel.Declaration.get_type decl) in
...
let vernac_printer x ident =
  let vernac_string = Pp.string_of_ppcmds x in
  Printf.printf "\nVernac Begin %s\n%s\nVernac End %s\n\n" ident vernac_string ident

let tactic_printer x =
  let tactic_string = Pp.string_of_ppcmds x in
  Printf.printf "\nTACTIC Begin \n%s\nTACTIC End \n\n" tactic_string

let pr_vernac_expr_subset v =
    let return = tag_vernac v in
    let env = Global.env () in
    let sigma = Evd.from_env env in
    match v with
      | VernacDefinition ((discharge,kind),id,b) -> (* A verifier... *)
        ...
        in
        let pr_lname_decl_def (n, u) =
          match n.CAst.v with
          | Anonymous -> str "_Anonymous"
          | Name id ->
              let full_path = Lib.make_path id in
              str (Libnames.string_of_path full_path) ++ pr_universe_decl u
        in
        let pr_reduce = function
          | None -> mt()
          | Some r ->
        ...
        return (
          hov 2 (
            pr_def_token kind ++ spc()
            ++ pr_lname_decl_def id ++ binds ++ typ
            ++ (match c with
              | None -> mt()
              | Some cc -> str" :=" ++ spc() ++ cc))
        ) in
        vernac_printer result "Definition";
        result

      | VernacStartTheoremProof (ki,l) ->
        let result =
          return (
          hov 1 (pr_statement (pr_thm_token_easy ki) (List.hd l) ++
                   prlist (pr_statement (spc () ++ keyword "with")) (List.tl l))
        ) in
        vernac_printer result "Proof";
        result

      | VernacAssumption ((discharge,kind),t,l) ->
        List.iter (fun (c, (xl, t')) ->
          List.iter (fun x ->
            let result =
              let (lid, univdecl) = x in
              let id = lid.CAst.v in
              let full_path = Lib.make_path id in
              let pr_single_param =
                hov 2 (str (Libnames.string_of_path full_path) ++ pr_universe_decl univdecl ++ spc() ++
                  (if c then str":>" else str":" ++ spc() ++ pr_lconstr_expr env sigma t')) in
              return (hov 2 (pr_assumption_token_easy false discharge kind ++
                            pr_non_empty_arg pr_assumption_inline t ++ spc() ++
                            pr_single_param))
            in
            vernac_printer result "Assumption"
          ) xl
        ) l;
        let result =
          let n = List.length (List.flatten (List.map fst (List.map snd l))) in
          let pr_params (c, (xl, t)) =
            let pr_fully_qualified_id x =
            let (lid, univdecl) = x in
            let id = lid.CAst.v in
            let full_path = Lib.make_path id in
            str (Libnames.string_of_path full_path) ++ pr_universe_decl univdecl
            in
            hov 2 (prlist_with_sep sep pr_fully_qualified_id xl ++ spc() ++
              (if c then str":>" else str":" ++ spc() ++ pr_lconstr_expr env sigma t)) in
          let assumptions = prlist_with_sep spc (fun p -> hov 1 (str "(" ++ pr_params p ++ str ")")) l in
          return (hov 2 (pr_assumption_token_easy (n > 1) discharge kind ++
                         pr_non_empty_arg pr_assumption_inline t ++ spc() ++ assumptions))
        in
        result
      ...
      | VernacInductive (cum, p,f,l) ->
        let result =
        let pr_constructor path (coe,(id,c)) =
          let constructor_name = Names.Id.to_string id.CAst.v in
          let full_path = (Libnames.string_of_path path) ^ "." ^ constructor_name in
          str "@IND Constructors Begin" ++ fnl() ++
          hov 2 (str full_path ++ str" " ++
                   (if coe then str":>" else str":") ++
                   Flags.without_option Flags.beautify pr_spc_lconstr c) ++
          fnl() ++ str "@IND Constructors End"
        in
        let pr_constructor_list path b l = match l with
          | Constructors [] -> mt()
          | Constructors l ->
            let fst_sep = match l with [_] -> "   " | _ -> " | " in
            pr_com_at (begin_of_inductive l) ++
              fnl() ++ str fst_sep ++
              prlist_with_sep (fun _ -> fnl() ++ str" | ") (pr_constructor path) l
          | RecordDecl (c,fs) ->
            pr_record_decl b c fs
        in
        let pr_oneind key (((coe,iddecl),indpar,s,k,lc),ntn) =
          let (lid, univdecl) = iddecl in
          let id = lid.CAst.v in
          let full_path = Lib.make_path id in
          hov 0 (
            str key ++ spc() ++
              (if coe then str"> " else str"") ++ str (Libnames.string_of_path full_path) ++ pr_universe_decl univdecl ++
              pr_and_type_binders_arg indpar ++
              pr_opt (fun s -> str":" ++ spc() ++ pr_lconstr_expr env sigma s) s ++
              str" :=") ++ pr_constructor_list full_path k lc ++
            prlist (pr_decl_notation @@ pr_constr env sigma) ntn
        in
        let key =
          let (_,_,_,k,_),_ = List.hd l in
      ...
      in
      vernac_printer result "Inductive";
      result
      ...
      | VernacFixpoint (local, recs) ->
        ...
        let local = match local with
          ...
          | NoDischarge -> ""
        in
        let pr_rec_definition_fix { fname; univs; rec_order; binders; rtype; body_def; notations } =
          let env = Global.env () in
          let sigma = Evd.from_env env in
          let id = fname.CAst.v in
          let full_path = Lib.make_path id in
          let pr_pure_lconstr c = Flags.without_option Flags.beautify pr_lconstr c in
          let annot = pr_guard_annot (pr_lconstr_expr env sigma) binders rec_order in
          str (Libnames.string_of_path full_path) ++ pr_universe_decl univs ++ pr_binders_arg binders ++ annot
          ++ pr_type_option (fun c -> spc() ++ pr_lconstr_expr env sigma c) rtype
          ++ pr_opt (fun def -> str":=" ++ brk(1,2) ++ pr_pure_lconstr env sigma def) body_def
          ++ prlist (pr_decl_notation @@ pr_constr env sigma) notations
        in
        return (
          hov 0 (str local ++ keyword "Fixpoint" ++ spc () ++
                   prlist_with_sep (fun _ -> fnl () ++ keyword "with"
                     ++ spc ()) pr_rec_definition_fix recs)
        )
      in
      vernac_printer result "Fixpoint";
      result
      ...
      | VernacCoFixpoint (local, corecs) ->
        ...
        let pr_onecorec {fname; univs; binders; rtype; body_def; notations } =
          ...
          let full_path = Lib.make_path id in
            str (Libnames.string_of_path full_path) ++
            pr_universe_decl univs ++  spc() ++ pr_binders env sigma binders ++ spc() ++ str":" ++
            spc() ++ pr_lconstr_expr env sigma rtype ++
            pr_opt (fun def -> str":=" ++ brk(1,2) ++ pr_lconstr env sigma def) body_def ++
            prlist (pr_decl_notation @@ pr_constr env sigma) notations
        in
      ...
      in
      vernac_printer result "CoFixpoint";
      result
      ...
      | VernacInstance (instid, sup, cl, props, info) ->
        ...
        return (
          hov 1 (
            ...
            (match instid with
             | {loc; v = Name id}, l ->
              let full_path = Lib.make_path id in
              spc () ++ str (Libnames.string_of_path full_path) ++ pr_universe_decl l ++ spc ()
             | { v = Anonymous }, _ -> spc () ++ str "_Anonymous") ++
            pr_and_type_binders_arg sup ++
              str":" ++ spc () ++
              pr_constr env sigma cl ++ pr_hint_info (pr_constr_pattern_expr env sigma) info ++
              (match props with
        ...
        in
        vernac_printer result "Instance";
        result
      (* vernac arg will make some bugs *)
      (* | VernacArguments (q, args, more_implicits, mods) ->
        let result =
        return (
          hov 2 (
      ...
      in
      vernac_printer result "Arguments";
      result *)
      | VernacDeclareInstance (instid, sup, cl, info) ->
        let result =
        let (lid, univdecl) = instid in
        let id = lid.CAst.v in
        let full_path = Lib.make_path id in
        return (
          hov 1 (
            keyword "Declare Instance" ++ spc () ++ str (Libnames.string_of_path full_path) ++
            pr_universe_decl univdecl ++ spc () ++
            pr_and_type_binders_arg sup ++
              str":" ++ spc () ++
              pr_constr env sigma cl ++ pr_hint_info (pr_constr_pattern_expr env sigma) info)
      ...
      | VernacContext l ->
        let pr_binders_arg_context =
          let env = Global.env () in
          let sigma = Evd.from_env env in
          pr_non_empty_arg @@ Ppconstr.pr_binders_context env sigma
        in
        let split_binder binder =
          match binder with
          | CLocalAssum (nal, k, t) ->
              List.map (fun name -> CLocalAssum ([name], k, t)) nal
          | _ -> [binder]
        in
        let split_binders =
          List.flatten (List.map split_binder l)
        in
        List.iter (fun single_binder ->
          let result =
            return (
              hov 1 (
                keyword "Context" ++ pr_binders_arg_context [single_binder])
            )
          in
          vernac_printer result "Context"
        ) split_binders;
        let result =
          return (
            hov 1 (
              keyword "Context" ++ pr_binders_arg_context l)
          )
        in
        result
