...
let new_instance_common ~program_mode ~generalize env instid ctx cl =
  ...
  Pretyping.print_constr_title "Instance";
  Printf.printf "Class: %s\n" (match k.cl_impl with 
  | GlobRef.ConstRef kn -> Names.Constant.to_string kn
  | GlobRef.IndRef (ind,_) -> Names.MutInd.to_string ind
  | _ -> "<unknown>");
  (match instid with 
  | Name id -> 
  ...
  Printf.printf "Type: \n";
  Pretyping.print_type_func env' sigma (Id.of_string("_instance")) (it_mkProd_or_LetIn cty ctx);
  Pretyping.print_constr_end "Instance";
