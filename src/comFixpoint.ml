...
let do_fixpoint_common (fixl : Vernacexpr.fixpoint_expr list) =
  ...
  let (fix, pl, ctx, info as fix_data) = interp_fixpoint ~cofix:false fixl in
  let env = Global.env () in
  let sigma = Evd.from_env env in
  let (fixnames,_,fixdefs,fixtypes) = fix in
  List.iter2 (fun name (typ, def) ->
    Pretyping.print_constr_title "Fixpoint";
    let full_path = Lib.make_path name in
    let full_name = Libnames.string_of_path full_path in
    Printf.printf "Name: %s\n" full_name;
    Printf.printf "Type: \n";
    Pretyping.print_type_func env sigma (Id.of_string("_fixpoint")) (EConstr.of_constr typ);
    (match def with
     | Some body -> 
        Printf.printf "Body: \n";
        Pretyping.print_type_func env sigma (Id.of_string("_fixpoint_type")) (EConstr.of_constr body)
     | None -> Printf.printf "Body: <no body>\n");
    Pretyping.print_constr_end "Fixpoint"
  ) fixnames (List.combine fixtypes fixdefs);
  fixl, ntns, fix_data, List.map compute_possible_guardness_evidences info
...
let do_cofixpoint_common (fixl : Vernacexpr.cofixpoint_expr list) =
  ...
  let ntns = List.map_append (fun { Vernacexpr.notations } -> notations ) fixl in
  let ((fixnames,_,fixdefs,fixtypes) as fix, pl, ctx, info as cofix_data) = interp_fixpoint ~cofix:true fixl in
  
  (* Print debug info *)
  let env = Global.env () in
  let sigma = Evd.from_env env in
  List.iter2 (fun name (typ, def) ->
    Pretyping.print_constr_title "CoFixpoint";
    let full_path = Lib.make_path name in
    let full_name = Libnames.string_of_path full_path in
    Printf.printf "Name: %s\n" full_name;
    Printf.printf "Type: \n";
    Pretyping.print_type_func env sigma (Id.of_string("_cofix")) (EConstr.of_constr typ);
    (match def with
     | Some body -> 
        Printf.printf "Body: \n";
        Pretyping.print_type_func env sigma (Id.of_string("_cofix_type")) (EConstr.of_constr body)
     | None -> Printf.printf "Body: <no body>\n");
    Pretyping.print_constr_end "CoFixpoint"
  ) fixnames (List.combine fixtypes fixdefs);
