...
let start_lemma_com ~program_mode ~poly ~scope ~kind ?hook thms =
  ...
  let evd, thms = List.fold_left_map (fun evd ((id, _), (bl, t)) ->
    ...
    let print_type_info () =
      let typ = Evarutil.nf_evar evd (EConstr.it_mkProd_or_LetIn t' ctx) in
      Pretyping.print_constr_title "Proof";
      let full_path = Lib.make_path id.v in
      let full_name = Libnames.string_of_path full_path in
      Printf.printf "Name: %s\n" full_name;
      Printf.printf "Type:\n" ;
      Pretyping.print_type_func env evd (Id.of_string("_proof")) typ;
      Pretyping.print_constr_end "Proof"
    in
    print_type_info ();
