...
let pr_goal_selector ~toplevel s =
  ...
  let pr_glob_tactic_level env n t =
    ...
    let rec prtac n (t:glob_tactic_expr) =
      ...
      (* let pr = {
        pr_tactic = prtac;
        pr_constr = (fun env sigma c -> Printf.printf "DEBUG[CONSTR]\n"; (pr_and_constr_expr (pr_glob_constr_env env)) c);
        pr_dconstr = (fun env sigma c -> Printf.printf "DEBUG[DCONSTR]\n"; (pr_and_constr_expr (pr_glob_constr_env env)) c);
        pr_lconstr = (fun env sigma c -> Printf.printf "DEBUG[LCONSTR]\n"; (pr_and_constr_expr (pr_lglob_constr_env env)) c);
        pr_pattern = (fun env sigma c -> Printf.printf "DEBUG[PATTERN]\n"; (pr_pat_and_constr_expr (pr_glob_constr_env env)) c);
        pr_lpattern = (fun env sigma c -> Printf.printf "DEBUG[LPATTERN]\n"; (pr_pat_and_constr_expr (pr_lglob_constr_env env)) c);
        pr_constant = (fun x -> Printf.printf "DEBUG[CONSTANT]\n"; pr_or_var (pr_and_short_name (pr_evaluable_reference_env env)) x);
        pr_reference = (fun x -> Printf.printf "DEBUG[REFERENCE]\n"; pr_ltac_or_var (pr_located pr_ltac_constant) x);
        pr_name = (fun x -> Printf.printf "DEBUG[NAME]\n"; pr_lident x);
        pr_generic = (fun x -> Printf.printf "DEBUG[GENERIC]\n"; Pputils.pr_glb_generic x);
        pr_extend = (fun x -> Printf.printf "DEBUG[EXTEND]\n"; pr_glob_extend_rec prtac x);
        pr_alias = (fun x -> Printf.printf "DEBUG[ALIAS]\n"; pr_glob_alias prtac x);
      } in *)
