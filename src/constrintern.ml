...
let intern_context env impl_env binders =
  ...
  let open Environ in
  let () = ()
    (* let view = Globals.view globals in
    Printf.printf "\nConstants:\n";
    Cmap_env.iter (fun cst (body, _) ->
      Printf.printf "Constant: %s\n" (Names.Constant.to_string cst);
      match body with
      | { const_type = typ; const_body = body; _ } ->
          let econstr_typ = EConstr.of_constr typ in
          print_type_func env sigma (Id.of_string "_constant") econstr_typ;
          match body with
          | Declarations.Def b -> 
            let real_body = Mod_subst.force_constr b in
            let econstr_body = EConstr.of_constr real_body in
            Printf.printf "Body: \n";
            print_type_func env sigma (Id.of_string "_body") econstr_body
          | Declarations.OpaqueDef _  -> 
              Printf.printf "Body: <opaque>\n"
          | Declarations.Undef _ ->
              Printf.printf "Body: <undefined>\n"
          | Declarations.Primitive _ ->
              Printf.printf "Body: <primitive>\n"
    ) view.Globals.constants; *)
    
    (* Printf.printf "\nInductives:\n";
    Mindmap_env.iter (fun mind (body, _) ->
      (* 打印归纳类型族的信息 *)
      let first_ind = body.mind_packets.(0) in
      Printf.printf "\nInductive family: %s\n" (Names.MutInd.to_string mind);
      Printf.printf "Number of parameters: %d\n" body.mind_nparams;
      
      (* 打印每个互递归归纳类型 *)
      Array.iteri (fun i ind_body ->
        Printf.printf "\nType[%d]: %s\n" i (Id.to_string ind_body.mind_typename);
        
        (* 将 rel_context 转换为 EConstr *)
        let econstr_ctx = List.map (fun decl ->
          match decl with
          | Context.Rel.Declaration.LocalAssum(name, typ) ->
              Context.Rel.Declaration.LocalAssum(name, EConstr.of_constr typ)
          | Context.Rel.Declaration.LocalDef(name, def, typ) ->
              Context.Rel.Declaration.LocalDef(name, EConstr.of_constr def, EConstr.of_constr typ)
        ) ind_body.mind_arity_ctxt in
        
        (* 打印参数和类型 *)
        List.iter (fun decl ->
          match decl with
          | Context.Rel.Declaration.LocalAssum(name, typ) ->
              Printf.printf "Parameter: \n" ;
              print_type_func env sigma (Id.of_string "dummy") typ
          | Context.Rel.Declaration.LocalDef(name, def, typ) ->
              Printf.printf "Let: %s := \n"
                (match Context.binder_name name with 
                 | Names.Name id -> Id.to_string id 
                 | Names.Anonymous -> "_");
                print_type_func env sigma (Id.of_string "dummy") def;
                Printf.printf " : ";
                print_type_func env sigma (Id.of_string "dummy") typ
        ) econstr_ctx;
        
        (* 打印构造器 *)
        Array.iteri (fun j cons_name ->
          Printf.printf "Constructor[%d]: %s\n" j (Id.to_string cons_name);
          (* 打印构造器类型 *)
          Printf.printf "  Type: \n" ;
            print_type_func env sigma (Id.of_string "dummy") (EConstr.of_constr ind_body.mind_user_lc.(j))
        ) ind_body.mind_consnames;
        
        (* 打印归纳类型性质 *)
        Printf.printf "Sort: %s\n" 
          (match ind_body.mind_kelim with
           | Sorts.InProp -> "Prop"
           | Sorts.InSet -> "Set"
           | Sorts.InType -> "Type"
           | _ -> "Other");
        
        Printf.printf "Recursivity: %s\n"
          (match body.mind_finite with
           | Finite -> "Inductive"
           | CoFinite -> "CoInductive"
           | BiFinite -> "Non-recursive")
      ) body.mind_packets
    ) view.Globals.inductives *)
  in
