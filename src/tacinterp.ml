...
let debug msg =
  Feedback.msg_notice (str ("[DBG] " ^ msg) ++ fnl ())   

let print_type_func env sigma id ty =
  Pretyping.print_type_func env sigma id ty

let print_single_goal_state prefix num total gl =
  let env = Proofview.Goal.env gl in
  let sigma = Proofview.Goal.sigma gl in
  let hyps = Proofview.Goal.hyps gl in
  let concl = Proofview.Goal.concl gl in

  let print_hyps = List.map (fun decl ->
    match decl with
    | Context.Named.Declaration.LocalAssum (id, ty) ->
        (Names.Name (Context.binder_name id), ty, None)
    | Context.Named.Declaration.LocalDef (id, term, ty) ->
        (Names.Name (Context.binder_name id), ty, Some term)
  ) hyps in

  Feedback.msg_notice (str (Printf.sprintf "PS %s: Goal %d/%d" prefix num total) ++ fnl());
  Feedback.msg_notice (str "PS Begin Hyps" ++ fnl());
  List.iter (fun (name, ty, _) ->
    match name with
    | Names.Name id ->
        Feedback.msg_notice (str "PS Begin Context" ++ fnl());
        Feedback.msg_notice (str "Name: " ++ str (Names.Id.to_string id)  ++ fnl());
        Feedback.msg_notice (str "Original: " ++ Printer.pr_econstr_env env sigma ty ++ fnl());
        print_type_func env sigma (Names.Id.of_string("_hyp")) ty;
        Feedback.msg_notice (str "PS End Context" ++ fnl())
    | Names.Anonymous ->
        Feedback.msg_notice (str "PS Begin Context" ++ fnl());
        Feedback.msg_notice (str "_Anonymous" ++ str " : " ++ fnl());
        Feedback.msg_notice (str "Original: " ++ Printer.pr_econstr_env env sigma ty ++ fnl());
        print_type_func env sigma (Names.Id.of_string "_hyp") ty;
        Feedback.msg_notice (str "PS End Context" ++ fnl())
  ) print_hyps;
  Feedback.msg_notice (str "PS End Hyps" ++ fnl());

  Feedback.msg_notice (
    str "PS Begin Goal" ++ fnl()
  );
  Feedback.msg_notice (str "Original: " ++ Printer.pr_econstr_env env sigma concl ++ fnl());
  print_type_func env sigma (Id.of_string "_goal") concl;
  Feedback.msg_notice (str "PS End Goal" ++ fnl());
  Feedback.msg_notice (str (Printf.sprintf "PS End Goals %d/%d " num total) ++ fnl())

let goals_changed before_goal after_goal tactic_str =
  (* true *)
  (* we need idtac to init proofstate when prove a new theorem *)
  if String.trim tactic_str = "idtac" then true
  else
    let before_sigma = Proofview.Goal.sigma before_goal in
    let after_sigma = Proofview.Goal.sigma after_goal in
    (* let before_sigma = copy (Proofview.Goal.sigma before_goal) in
    let after_sigma = copy (Proofview.Goal.sigma after_goal) in *)

    let before_concl = Proofview.Goal.concl before_goal in
    let after_concl = Proofview.Goal.concl after_goal in

    let concl_changed =
    try
      not (EConstr.eq_constr before_sigma before_concl after_concl)
    with e ->
      raise e
    in
    let concl_changed =
      not (EConstr.eq_constr before_sigma before_concl after_concl) in

    let hyps_before = Proofview.Goal.hyps before_goal in
    let hyps_after = Proofview.Goal.hyps after_goal in
    let hyps_changed = hyps_before <> hyps_after in

    concl_changed || hyps_changed

let remove_newlines str =
    String.map (fun c -> if c = '\n' then ' ' else c) str

let execute_with_state_tracking ~print name tactic_str (tactic : unit Proofview.tactic) : unit Proofview.tactic =
  let handle_tactic theorem_name tactic =
    let tactic_str = remove_newlines tactic_str in

    let print_msg msg = if print then Feedback.msg_notice msg else () in

    Proofview.Goal.goals >>= fun before_goals ->
  
    let theorem_name =
      try
        let current_id = Vernacstate.Proof_global.get_current_proof_name () in
        let full_path = Lib.make_path current_id in
        String.map (function '\n' -> ' ' | c -> c)
          (Libnames.string_of_path full_path)
      with Vernacstate.Proof_global.NoCurrentProof -> "Unnamed_thm"
    in
    (if print then
      Proofview.tclUNIT (
        print_msg (str (Printf.sprintf 
          "PS Begin of %s <T>%s</T> in proof of %s"
          name tactic_str theorem_name) ++ fnl())
      )
    else Proofview.tclUNIT ()) >>= fun () ->

    let main_tactic = 
      let open Proofview.Notations in
      tactic >>= fun () ->
      Proofview.Goal.goals >>= fun after_goals ->

      let goals_count_changed = List.length before_goals <> List.length after_goals in
    
      let check_individual_goals_changed () =
        let rec check before_list after_list =
          match before_list, after_list with
          | [], [] -> Proofview.tclUNIT false
          | [], _ | _, [] -> Proofview.tclUNIT true
          | g1::gs1, g2::gs2 ->
              g1 >>= fun before_goal ->
              g2 >>= fun after_goal ->
              if goals_changed before_goal after_goal tactic_str then
                Proofview.tclUNIT true
              else
                check gs1 gs2
        in
        check before_goals after_goals
      in
      
      (if goals_count_changed then
        Proofview.tclUNIT true
      else
        check_individual_goals_changed ()) >>= fun state_changed ->

      (if print then
        if state_changed then
          match before_goals with
          | [] -> Proofview.tclUNIT ()
          | g::gs -> 
            g >>= fun first_goal ->
            Proofview.tclUNIT (
              print_single_goal_state 
                (Printf.sprintf "Before %s <T>%s</T> in proof of %s"
                  name tactic_str theorem_name) 1 (List.length before_goals) first_goal
            ) >>= fun () ->
            let rec print_remaining_goals num = function
              | [] -> Proofview.tclUNIT ()
              | g::gs ->
                  g >>= fun goal ->
                  Proofview.tclUNIT (
                    print_single_goal_state 
                      (Printf.sprintf "Before %s <T>%s</T> in proof of %s"
                        name tactic_str theorem_name) num (List.length before_goals) goal
                  ) >>= fun () ->
                  print_remaining_goals (num+1) gs
            in
            print_remaining_goals 2 gs >>= fun () ->

          match after_goals with
          | [] ->
              Proofview.tclUNIT (
                Feedback.msg_notice (str (Printf.sprintf 
                  "PS After %s <T>%s</T>: in proof of %s"
                  name tactic_str theorem_name) ++ fnl());
                Feedback.msg_notice (str "PS Current subgoal Completed" ++ fnl());
                Feedback.msg_notice (str (Printf.sprintf
                  "PS End of %s <T>%s</T> with 0 remaining goals in proof of %s"
                  name tactic_str theorem_name) ++ fnl())
              )
          | goals ->
              let total = List.length goals in
              let rec print_all_goals num = function
                | [] -> Proofview.tclUNIT ()
                | g::gs ->
                    g >>= fun goal ->
                    Proofview.tclUNIT (
                      print_single_goal_state 
                        (Printf.sprintf "After %s <T>%s</T> in proof of %s"
                          name tactic_str theorem_name) num total goal
                    ) >>= fun () ->
                    print_all_goals (num+1) gs
              in
              print_all_goals 1 goals >>= fun () ->
              Proofview.tclUNIT (
                Feedback.msg_notice (str (Printf.sprintf
                  "PS End of %s <T>%s</T> with %d remaining goals in proof of %s"
                  name tactic_str total theorem_name) ++ fnl())
              )
            else
              Proofview.tclUNIT (
                print_msg (str (Printf.sprintf
                  "PS Notice: Tactic <T>%s</T> did not change the proof state in proof of %s"
                  tactic_str theorem_name) ++ fnl())
              )
        else
        Proofview.tclUNIT ())
      in

      let error_handler (e, info) =
        if print then
          Proofview.tclUNIT (
            Feedback.msg_notice (str (Printf.sprintf
              "PS Notice: Failed to execute <T>%s</T>"
              tactic_str) ++ fnl())
          ) >>= fun () ->
          ...
          Proofview.tclZERO ~info e
      in
      Proofview.tclOR main_tactic error_handler
  in
  ...
  try
    let theorem_name =
      ...
      with Vernacstate.Proof_global.NoCurrentProof -> "Unnamed_thm"
    in
    let tactic_str = remove_newlines tactic_str
    in
    let is_in_proof =
      try
        let _ = Vernacstate.Proof_global.give_me_the_proof () in
        true
      with Vernacstate.Proof_global.NoCurrentProof -> false
    in
    if is_in_proof then
      handle_tactic theorem_name tactic
    else
      tactic
  with e ->
    if print then
    Feedback.msg_notice (str (Printf.sprintf "PS Notice: Exception in execute_with_state_tracking: %s"
      (Printexc.to_string e)));
    tactic

let execute_with_state_tracking_linear ~print name tactic_str tactic gl =
  let handle_tactic theorem_name tactic gl =
    let tactic_str = remove_newlines tactic_str in
    let before_goal = gl in
    ...
    let main_tactic = 
      ...
      match goals with
      | [] ->
        (* subgoal completed, state must be changed *)
        Proofview.tclUNIT (
          if print then begin
          (* Feedback.msg_notice (str (Printf.sprintf "PS Executing tactic <T>%s</T> in proof of %s"
            tactic_str theorem_name) ++ fnl()); *)
          print_single_goal_state (Printf.sprintf "Before %s <T>%s</T> in proof of %s"
            name tactic_str theorem_name) 1 1 gl;
          print_msg  (str (Printf.sprintf "PS After %s <T>%s</T>: in proof of %s"
            name tactic_str theorem_name) ++ fnl());
          print_msg  (str "PS Current subgoal Completed" ++ fnl());
          print_msg (str (Printf.sprintf
          "PS End of %s <T>%s</T> with 0 remaining goals in proof of %s"
          name tactic_str theorem_name) ++ fnl())
          end
      ...
      | g::gs ->
        g >>= fun after_goal ->
          let multiple_goals = List.length gs > 0 in
          if multiple_goals || goals_changed before_goal after_goal tactic_str then
          let total = List.length goals in
          (* Proofview.tclUNIT (
            Feedback.msg_notice (str (Printf.sprintf "PS Executing tactic <T>%s</T> in proof of %s"
              tactic_str theorem_name) ++ fnl())
          ...
          let rec print_all_goals num = function
            ...
            | g::gs ->
                ...
                Proofview.tclUNIT (
                  if print then
                  print_single_goal_state (Printf.sprintf "After %s <T>%s</T> in proof of %s"
                    name tactic_str theorem_name) num total goal
                ) >>= fun () ->
                print_all_goals (num+1) gs
          in
    ...
    in
    (* for some tactic can not be used *)
    let error_tactic (e, info) =
      let open Proofview.Notations in
      Proofview.tclUNIT (
        print_msg (str (Printf.sprintf
          "PS Notice: Failed to execute <T>%s</T> in proof of %s"
          tactic_str theorem_name) ++ fnl())
      ) >>= fun () ->
      Proofview.tclZERO ~info e
    in
    Proofview.tclOR main_tactic error_tactic
  in
  ...
  try
    ...
    in
    if is_in_proof then
      handle_tactic theorem_name tactic gl
    else
      tactic
  with e ->
    if print then
...
let robust_tactic_execution ~print name tactic_str tactic =
  Proofview.tclORELSE
    (Proofview.Goal.enter (fun gl ->
      execute_with_state_tracking_linear
        ~print name tactic_str tactic gl))
    (fun (e, info) ->
      if print then
        (Feedback.msg_notice (str (Printf.sprintf 
          "Linear tracking failed for <T>%s</T>, falling back to global tracking." 
          tactic_str) ++ fnl());
        execute_with_state_tracking ~print name tactic_str tactic)
      else
        execute_with_state_tracking ~print name tactic_str tactic)
...
let default_depth = 0
let is_expanding_ltac = ref false
let use_linear_execution = ref true
let print_tactic_str = ref true

let glob_depth = ref 2

let () =
  let open Goptions in
  declare_bool_option
    { optdepr  = false;
      optname  = "linear execution mode for tactics";
      optkey   = ["Linear";"Execution"];
      optread  = (fun () -> !use_linear_execution);
      optwrite = (fun b -> use_linear_execution := b) };
  declare_int_option
    { optdepr  = false;
      optname  = "tactic evaluation depth";
      optkey   = ["Tactic";"Depth"];
      optread  = (fun () -> Some !glob_depth);
      optwrite = (fun d -> match d with Some v -> glob_depth := v | None -> ()) };
  declare_bool_option
    { optdepr  = false;
      optname  = "Print tactic string";
      optkey   = ["PrintTacticStr"];
      optread  = (fun () -> !print_tactic_str);
      optwrite = (fun b -> print_tactic_str := b) }
...
let rec val_interp ist ?(appl=UnnamedAppl) (tac:glob_tactic_expr) : Val.t Ftactic.t =
  ...
  | TacFun (it, body) ->
    (* Printf.printf "[VAL_INTERP:FUN] Entering TacFun branch\n"; *)
    is_expanding_ltac := true;
    Ftactic.return (of_tacvalue (VFun (UnnamedAppl,extract_trace ist, ist.lfun, it, body)))
  | TacLetIn (true,l,u) -> 
    (* Printf.printf "[VAL_INTERP:LETREC] Entering TacLetIn(recursive) branch\n"; *)
    is_expanding_ltac := true;
    interp_letrec ist l u
  | TacLetIn (false,l,u) -> 
    (* Printf.printf "[VAL_INTERP:LETIN] Entering TacLetIn(non-recursive) branch\n"; *)
    is_expanding_ltac := true;
    interp_letin ist l u
  | TacMatchGoal (lz,lr,lmr) -> 
    (* Printf.printf "[VAL_INTERP:MATCHGOAL] Entering TacMatchGoal branch\n"; *)
    is_expanding_ltac := true;
    interp_match_goal ist lz lr lmr
  | TacMatch (lz,c,lmr) -> 
    (* Printf.printf "[VAL_INTERP:MATCH] Entering TacMatch branch\n"; *)
    is_expanding_ltac := true;
    interp_match ist lz c lmr
  | TacArg {loc;v} -> 
    (* Printf.printf "[VAL_INTERP:ARG] Entering TacArg branch\n"; *)
    is_expanding_ltac := true;
    interp_tacarg ist v
  | t ->
    (* Delayed evaluation *)
    (* Printf.printf "[VAL_INTERP:OTHER] Entering catch-all branch\n"; *)
...
and eval_tactic ?(depth=default_depth) ?(expanding_ltac=false) ist tac : unit Proofview.tactic =
  (* let tactic_str = (Pp.string_of_ppcmds (
    Pptactic.pr_glob_tactic (Global.env()) tac)) in
  Printf.printf "[DEBUGDEBUG] Executing tactic: %s\n" tactic_str; *)
  let current_depth = if depth > 0 then depth - 1 else 0 in
  let depth_to_thens = 
    if expanding_ltac then
      current_depth
    else
      depth
    in 
  ...
  | TacAtom {loc;v=t} ->
      ...
      Profile_ltac.do_profile "eval_tactic:2" trace
        (catch_error_tac ?loc trace (interp_atomic ~depth:current_depth ist t))
  | TacFun _ | TacLetIn _ | TacMatchGoal _ | TacMatch _ -> interp_tactic ~depth:current_depth ist tac
  ...
  | TacFail (g,n,s) ->
      ...
      in
      Ftactic.run msg tac
  | TacProgress tac -> Tacticals.New.tclPROGRESS (interp_tactic ~depth:current_depth ist tac)
  | TacShowHyps tac ->
         Proofview.V82.tactic begin
           tclSHOWHYPS (Proofview.V82.of_tactic (interp_tactic ~depth:current_depth ist tac))
         end
  | TacAbstract (t,ido) ->
      ...
      Proofview.Goal.enter begin fun gl -> Abstract.tclABSTRACT
        (Option.map (interp_ident ist (pf_env gl) (project gl)) ido) (interp_tactic ~depth:current_depth ist t)
      end end)
  | TacThen (t1,t) ->
      Tacticals.New.tclTHEN (interp_tactic ~depth:depth_to_thens ist t1) (interp_tactic ~depth:depth_to_thens ist t)
  | TacDispatch tl ->
      Proofview.tclDISPATCH (List.map (interp_tactic ~depth:current_depth ist) tl)
  | TacExtendTac (tf,t,tl) ->
      Proofview.tclEXTEND (Array.map_to_list (interp_tactic ~depth:current_depth ist) tf)
                          (interp_tactic ~depth:current_depth ist t)
                          (Array.map_to_list (interp_tactic ~depth:current_depth ist) tl)
  | TacThens (t1,tl) -> 
      Tacticals.New.tclTHENS (interp_tactic ~depth:depth_to_thens ist t1) (List.map (interp_tactic ~depth:depth_to_thens ist) tl)
  | TacThens3parts (t1,tf,t,tl) ->
      Tacticals.New.tclTHENS3PARTS (interp_tactic ~depth:depth_to_thens ist t1)
        (Array.map (interp_tactic ~depth:depth_to_thens ist) tf) (interp_tactic ~depth:depth_to_thens ist t) (Array.map (interp_tactic ~depth:depth_to_thens ist) tl)
  | TacDo (n,tac) -> Tacticals.New.tclDO (interp_int_or_var ist n) (interp_tactic ~depth:current_depth ist tac)
  | TacTimeout (n,tac) -> Tacticals.New.tclTIMEOUT (interp_int_or_var ist n) (interp_tactic ~depth:current_depth ist tac)
  | TacTime (s,tac) -> Tacticals.New.tclTIME s (interp_tactic ~depth:current_depth ist tac)
  | TacTry tac -> Tacticals.New.tclTRY (interp_tactic ~depth:current_depth ist tac)
  | TacRepeat tac -> Tacticals.New.tclREPEAT (interp_tactic ~depth:current_depth ist tac)
  | TacOr (tac1,tac2) ->
      Tacticals.New.tclOR (interp_tactic ~depth:current_depth ist tac1) (interp_tactic ~depth:current_depth ist tac2)
  | TacOnce tac ->
      Tacticals.New.tclONCE (interp_tactic ~depth:current_depth ist tac)
  | TacExactlyOnce tac ->
      Tacticals.New.tclEXACTLY_ONCE (interp_tactic ~depth:current_depth ist tac)
  | TacIfThenCatch (t,tt,te) ->
      Tacticals.New.tclIFCATCH
        (interp_tactic ~depth:current_depth ist t)
        (fun () -> interp_tactic ~depth:current_depth ist tt)
        (fun () -> interp_tactic ~depth:current_depth ist te)
  | TacOrelse (tac1,tac2) ->
      Tacticals.New.tclORELSE (interp_tactic ~depth:current_depth ist tac1) (interp_tactic ~depth:current_depth ist tac2)
  | TacFirst l -> Tacticals.New.tclFIRST (List.map (interp_tactic ~depth:current_depth ist) l)
  | TacSolve l -> Tacticals.New.tclSOLVE (List.map (interp_tactic ~depth:current_depth ist) l)
  | TacComplete tac -> Tacticals.New.tclCOMPLETE (interp_tactic ~depth:current_depth ist tac)
  | TacArg a -> interp_tactic ~depth:current_depth ist (TacArg a)
  | TacInfo tac ->
      warn_deprecated_info ();
      eval_tactic ~depth:current_depth ~expanding_ltac:expanding_ltac ist tac
  | TacSelect (sel, tac) -> Tacticals.New.tclSELECT sel (interp_tactic ~depth:current_depth ist tac)
  (* For extensions *)
  | TacAlias {loc; v=(s,l)} ->
      (* Printf.printf "[ALIAS] Executing Ltac tactic: %s\n"
      (Pp.string_of_ppcmds (
      ...
      let tac l =
        ...
        Ftactic.lift (catch_error_loc ?loc (tactic_of_value ~depth:current_depth ~expanding_ltac:expanding_ltac ist v))
      in
      let tac =
...
and tactic_of_value ?(depth=default_depth) ?(expanding_ltac=false) ist vle =
  ...
  | VFun (appl,trace,lfun,[],t) ->
    Proofview.tclProofInfo [@ocaml.warning "-3"] >>= fun (_name, poly) ->
      ...
      let tac = name_if_glob appl (eval_tactic ~depth:depth ~expanding_ltac:expanding_ltac ist t) in
  ...
  | VRec _ -> Tacticals.New.tclZEROMSG (str "A fully applied tactic is expected.")
    ...
    tactic_of_value ~depth:depth ~expanding_ltac:expanding_ltac ist tac
...
and interp_match_success ist { Tactic_matching.subst ; context ; terms ; lhs } =
  ...
  | VFun (appl,trace,lfun,[],t) ->
      ...
      let tac = eval_tactic ~depth:(if !glob_depth > 2 then 2 else 0) ~expanding_ltac:true ist t in
...
and interp_tactic ?(depth=default_depth) ist tac : unit Proofview.tactic =
  let tactic_str =
    if !print_tactic_str then
      String.map (function '\n' -> ' ' | c -> c)
        (Pp.string_of_ppcmds (
          Pptactic.pr_glob_tactic (Global.env()) tac))
    else
  ...
  in
  let original_tac = 
    Ftactic.run (val_interp ist tac) (fun v -> 
      let was_expanding = !is_expanding_ltac in
      is_expanding_ltac := false;
      tactic_of_value ~depth:depth ~expanding_ltac:was_expanding ist v) in

  if depth > 0 then begin
    Printf.printf "[TACTIC] Executing tactic: %s\n" tactic_str;
    if !use_linear_execution then
      (
        (* Printf.printf "[TACTIC] Using linear execution\n"; *)
      Proofview.Goal.enter (fun gl ->
        execute_with_state_tracking_linear
          ~print:true "TACTIC" tactic_str original_tac gl))
    else
      (
        Printf.printf "[DEBUG] Using non-linear execution\n";
      execute_with_state_tracking
        ~print:true "TACTIC" tactic_str original_tac)
  end else (
    original_tac )
...
and interp_atomic ?(depth=default_depth) ist tac : unit Proofview.tactic =
  match tac with
  (* Basic tactics *)
  ...
  | TacAssert (ev,b,t,ipat,c) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        in
        let sigma, ipat' = interp_intro_pattern_option ist env sigma ipat in
        let tac = Option.map (Option.map (interp_tactic ~depth:(if !glob_depth > 2 then depth else 0) ist)) t in
        Tacticals.New.tclWITHHOLES ev
        (name_atomic ~env
      ...
      Proofview.Goal.enter begin fun gl ->
        ...
        name_atomic ~env
          ...
          (Equality.general_multi_rewrite ev l' cl
             (Option.map (fun by -> Tacticals.New.tclCOMPLETE (interp_tactic ~depth:(if !glob_depth > 2 then depth else 0) ist by),
               Equality.Naive)
                by))
      end
...
let eval_tactic t =
  ...
  interp_tactic ~depth:!glob_depth (default_ist ()) t

let eval_tactic_ist ist t =
  Proofview.tclLIFT db_initialize <*>
  interp_tactic ~depth:0 ist t
