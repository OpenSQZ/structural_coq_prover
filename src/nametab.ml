...
let pr_global_env env ref =
  try 
  let path = path_of_global ref in
  let dir = dirpath path in
  let base = basename path in
  (* Feedback.msg_notice (Pp.str (Printf.sprintf "Full path - Dir: %s, Base: %s" 
    (DirPath.to_string dir) (Id.to_string base))); *)
  pr_qualid (make_qualid dir base)
  with Not_found as e ->
