...
let do_program_recursive ~scope ~poly fixkind fixl =
  ...
  List.iter2 (fun name (typ, def) ->
    Pretyping.print_constr_title (if cofix then "CoFixpoint" else "Fixpoint");
    ...
    (* Print type *)
    Printf.printf "Type:\n";
    Pretyping.print_type_func env evd (Id.of_string("_fixpoint")) (EConstr.of_constr typ);
    
    (* Print body *)
    Printf.printf "Body:\n";
    Pretyping.print_type_func env evd (Id.of_string("_fixpoint_body")) (EConstr.of_constr def);
    
    Pretyping.print_constr_end (if cofix then "CoFixpoint" else "Fixpoint")
  ) fixnames (List.map (fun (typ, def) -> 
    (EConstr.to_constr evd typ, EConstr.to_constr evd def)
  ) (List.combine fixtypes fixdefs));
