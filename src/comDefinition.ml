...
let do_definition ~program_mode ?hook ~name ~scope ~poly ~kind univdecl bl red_option c ctypopt =
  ...
  let sigma = evd in
  let print_def_info () =
    let (body, ctx), _ = Future.force ce.Declare.proof_entry_body in
    Pretyping.print_constr_title "Definition";
    let mp = Lib.current_mp () in
    let full_path = Lib.make_path name in
    let full_name = Libnames.string_of_path full_path in
    Printf.printf "Name: %s\n\n" full_name;
    (match ce.Declare.proof_entry_type with
    | Some typ -> 
        Printf.printf "Type: \n";
        print_type_func env sigma (Names.Id.of_string("_definition")) (EConstr.of_constr typ)
    | None -> 
        let body_econstr = EConstr.of_constr body in
        let typ = Retyping.get_type_of env sigma body_econstr in
        Printf.printf "Type: \n";
        print_type_func env sigma (Names.Id.of_string("_definition")) typ);
    Printf.printf "Body: \n";
    print_type_func env sigma (Names.Id.of_string("_definition_body")) (EConstr.of_constr body);
    Pretyping.print_constr_end "Definition";
  in
  print_def_info ();
  (* let (ce, evd, univdecl, imps as def) =
    interp_definition ~program_mode univdecl bl ~poly red_option c ctypopt
